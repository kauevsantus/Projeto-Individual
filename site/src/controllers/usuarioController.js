...
function inserir(req, res) {
    ...
    var album = req.body.fkAlbumServer;
    if (totalPontos == undefined) {
        alert('Seus pontos não estão definidos')

    } else {
        usuarioModel.inserir(totalPontos, idUsuario, album)
        .then(
            function(resultado){
                res.json(resultado);
            } 
...
function resultado(req, res) {
    ...
    // puxando sessionstorage da quiz.js
    var idUsuario = req.params.idUsuario;


    usuarioModel.resultado(idUsuario).then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!")
